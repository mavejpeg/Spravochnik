    ...
    accordions.forEach(acc => {
        if (!acc.classList.contains('converted')) {
            ...
            if (header && body) {
                ...
                const bodyContent = body.cloneNode(true);
        ...
        <div class="modal" style="max-width: 1000px; max-height: 90vh; overflow-y: auto;">
            ...
            <div class="modal-body">
                ...
                <div class="editor-toolbar">
                    ...
                    <button class="tool-btn" data-action="add-list">📋 Добавить список (маркированный)</button>
                    <button class="tool-btn" data-action="add-numbered-list">🔢 Добавить список (нумерованный)</button>
        ...
        return `
            <div class="editor-item dropdown-item" data-type="dropdown">
                ...
                <div class="editor-item-content">
                    ...
                    <div class="rich-editor-container" data-type="dropdown-content">
            ...
            <div class="editor-item card-item" data-type="card">
                ...
                <div class="editor-item-content">
                    ...
                    <div class="list-editor" data-type="unordered">
                        <div class="list-items">${items.split('\n').map(item => `<div class="list-item"><input type="text" value="${escapeHtml(item)}" placeholder="Пункт списка"><button class="remove-list-item">✕</button></div>`).join('')}</div>
                    ...
                    <div class="editor-item-content">
                        <div class="list-editor" data-type="unordered">
                            <div class="list-items">
                                <div class="list-item"><input type="text" placeholder="Пункт списка"><button class="remove-list-item">✕</button></div>
                            </div>
                            <button class="add-list-item">➕ Добавить пункт</button>
                        </div>
                    </div>
    ...
    richEditors.forEach(editor => {
        ...
        if (toolbar) {
            toolbar.querySelectorAll('.format-btn').forEach(btn => {
                btn.removeEventListener('click', handleRichFormat);
                btn.addEventListener('click', (e) => {
                    e.preventDefault();
...
function handleAddStep(e) {
    const editor = e.target.closest('.editor-item-content')?.querySelector('.steps-editor');
    if (editor) {
        const stepNum = editor.querySelectorAll('.step-item').length + 1;
        const newStep = document.createElement('div');
        newStep.className = 'step-item';
...
function handleRemoveStep(e) {
    ...
    const editor = e.target.closest('.steps-editor');
                ...
                if (dropdownTitle && dropdownContent) {
                    html += `
                        <details style="background: var(--surface); border: 1px solid var(--border); border-radius: 12px; margin-bottom: 10px;">
                            <summary style="cursor: pointer; padding: 12px 16px; font-weight: 600; color: var(--accent); list-style: none; display: flex; justify-content: space-between; align-items: center;">
                                ${escapeHtml(dropdownTitle)}<span style="font-size: 12px;">▼</span>
                            </summary>
                            ${dropdownContent}
                        </details>
                    `;
