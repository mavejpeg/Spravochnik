// main.js - исправленная версия (удалён дублирующийся код редактора)
let isRop = false;
...
async function loadUserInfo() {
    try {
        ...
        if (data.authenticated) {
            ...
            // Настройка кнопок редактирования (функция из editor.js)
            if (typeof setupEditButtons === 'function') {
                setupEditButtons();
            } else {
                console.log('setupEditButtons not available yet, will retry');
                setTimeout(() => {
...
// Экспортируем openRopPanel в глобальный объект для доступа из других файлов
